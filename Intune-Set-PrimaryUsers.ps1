...
# Customizations
$Enrollmentaccounts = @("IntuneEnrollment@hbs.net") # @() = No Enrollment accounts. @("user@companydomain.com","user2@companydomain.com") = will filter them out and not assign them as primary users.
$AzureAutomation = $False                # $True = The script will be executed in Azure Automation. $False The script will be executed manually 
$ExecutionMode = "Test"                 # "Test" = No changes will be made on Primary owner, "Prod" = Primary Owner will be changes
$VerbosePreference = "Continue" # "SilentlyContinue" = Doesn't display the verbose message. Continues executing. "Continue" = Show Verbose Messages
...
Start-Transcript -Path ".\Intune-SetPrimaryUsser.log" -Force -Confirm:$false
